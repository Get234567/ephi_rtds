...
class AuditReader extends BaseAuditReader
{
     ...
     * @return Pagerfanta
     */
    public function getAuditsPagerSearch($entity, $id = null, int $page = 1, int $pageSize = self::PAGE_SIZE, $user = null, $ops = null): Pagerfanta
    {
        $queryBuilder = $this->getAuditsQueryBuilderSearch($entity, $id, $user, $page, $pageSize, $ops);
     ...
     * @return QueryBuilder
     */
    private function getAuditsQueryBuilderSearch($entity, $id = null, $user = null, ?int $page = null, ?int $pageSize = null, $ops = null): QueryBuilder
    {
        if (null !== $page && $page < 1) {
            throw new \InvalidArgumentException('$page must be greater or equal than 1.');
        }
        ...
        if (null !== $user) {
            $queryBuilder
                ->andWhere('blame_user = :object_id')
                ->setParameter('object_id', $user);
        }
        if (null !== $ops) {
            if ($ops == 'all') {
            } else {
